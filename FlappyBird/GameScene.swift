...
//  FlappyBird
//
//  Created by Shigeo Kikuchi on 2017/01/06.
//  Copyright © 2017年 shigeo.kikuchi. All rights reserved.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory {
            ...
            // 音を鳴らす
            let sound = SKAction.playSoundFileNamed("pup.mp3", waitForCompletion: false)
            self.run(sound)
    ...
    func setupItem() {
        ...
        let createItemAnimation = SKAction.run({
            ...
            under.position = CGPoint(x: 0.0, y: item_y)
            item.addChild(under)
